	...
	public function sql_exec($sql_line, $sql_type, $import_table_prefix = '') {
		...
		if (!$req) {
			...
			if (1 == $this->errors && 2 == $sql_type && 0 == $this->tables_created) {
				...
				$this->maintenance_mode(false);
				return new WP_Error('initial_db_error', sprintf(__('An error occurred on the first %s command - aborting run','updraftplus'), 'SET NAMES').' '.sprintf(__('To use this backup, database server needs to support the %s character set.', 'updraftplus'), $this->set_names));
			}
